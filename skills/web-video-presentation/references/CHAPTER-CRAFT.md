...
- 至少一个对象 / 线 / 数据包 / 镜头框 / 页面 / 形状跨相邻 step 延续
- 文字只是标签和强调，不是唯一画面主体
...
`step` 是口播节拍，不一定等于"换一整屏"。根据内容选择：

| 模式 | 适合什么 | 做法 |
|---|---|---|
| 全屏镜头 | 开场 hook、强转折、章节幕封、概念冲击、故事段落切换 | 一个 step 控制一组主视觉 / 镜头构图 |
| 复杂页面讲解 | 产品界面、系统架构、流程图、数据看板、代码 / 工具链讲解 | 同一页保留多个区域，step 只改变焦点、高亮、连线、状态和局部运动 |

复杂页面讲解详见 [`COMPLEX-PAGE-STEPS.md`](COMPLEX-PAGE-STEPS.md)。基本要求：

- 页面主体要持续存在，观众能建立空间记忆。
- 每个 step 只聚焦一个区域 / 状态变化，其它区域降噪但保留上下文。
- 已讲过的区域进入完成态，未讲区域保持低对比，不要让整页同时抢注意力。
- 用焦点框、信号线、数据包、扫线、路径自绘或状态变化，把视线带到当前讲解点。
...
- 如果采用复杂页面讲解，当前 step 是否只有一个明确焦点？
- 如果录屏时没有鼠标光标，画面还像一条视频吗？

有一条答不上来，就回去改这一 step 的视觉动作。
