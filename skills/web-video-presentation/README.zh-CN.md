...
`web-video-presentation` 帮 Agent 构建一种 Vite + React + TypeScript 演示：它看起来不是传统幻灯片，而更像为录屏设计的视频舞台。每次点击推进一个口播节拍；step 可以独占 1920×1080 舞台，也可以在同一个复杂页面里逐区聚焦。进度 UI 平时隐藏，只有悬浮时出现，方便录出干净画面。
...
- **一步一个想法**：每个节拍只有一个视觉焦点，可以换整屏，也可以在复杂页面里移动焦点。
